...
<head>
<meta charset="UTF-8">
<title>Quản Lý Phiếu Nhập | Admin TH-Homeware</title>
					...
					<div class="col-md-12">
						...
						<div class="btn btn-success" style="padding: 0px 0px;" >
							...
							<!-- <button data-button="addReceipt" id="btnAddReceipt" class="btn btn-success" style="color: white;text-transform:uppercase;font-size:14px;">Thêm Phiếu Nhập</button> -->
								 <a class="navbar-brand" href="/TH_Homeware/Admin/goodreceipt_insert.jsp" style="color: white;text-transform:uppercase;font-size:14px;"> Thêm Phiếu Nhập</a> 
						</div>
					</div>
				</div>
				<hr />
				...
				<div class="row">
					...
					<%
						...
						<td class="colInfo"><%=nf.format(gr.getTotal())%> VNĐ
							...
							<a href="/TH_Homeware/ReceiptServlet_View?action=Views&ID_Receipt=<%=gr.getID_Receipt()%>"
						...
						<td style="text-align:center">
							 <a <% if(gr.getTotal() > 0 || !user.getID_Account().equals(a)){ %> style="display:none"<%}else{ %> style=" display: block;padding:3px 0px;margin-left:30%;width:35%"
									...
									%> 
							href="/TH_Homeware/ReceiptServlet_Delete?action=Delete&ID_Receipt=<%=gr.getID_Receipt()%>" onclick="return confirm('Bạn có chắc chắn xóa mã phiếu nhập là <%=gr.getID_Receipt()%> không ?')"
